...
from infinity.infinity_api_v1 import (
    ...
    inf_mai_total,
    inf_mai_level,
    inf_mai_seek_aliases,
    inf_mai_roll,
    inf_mai_roll_by_decimal,
    inf_mai_roll_by_level,
    inf_mai_plate_song_list_v1,
    inf_mai_plate_requirement,
    inf_mai_ra_calculating,
    inf_mai_score_line_v1,
    inf_mai_course,
    inf_mai_shincourse,
    inf_mai_b50_v1,
    inf_mai_ap50_v1,
    inf_mai_plate_completion_v1,
    inf_mai_plate_process_v1,
    inf_mai_level_score_list_v1,
    inf_mai_score_v1,
)
...
mai_command = on_alconna(
    Alconna(
        ...
        Subcommand("分数线", Args["music_id", int]["difficulty?", str]),
        Subcommand("段位认定"),
        Subcommand("真段位认定"),
        Subcommand("随机段位认定", Args["type?", str]),
        Subcommand("友人对战"),
        Subcommand("查找机厅", Args["keyword", str]),
        Subcommand(
            "b50",
            Args["username?", str],
...
what_song_cmd = on_alconna(
    Alconna(
        "mai {keyword:str}是什么歌", Option("-p|--page", Args["page", int], default=1)
    ),
    use_cmd_start=True,
    rule=is_allowed,
)
...
version_song_list_cmd = on_alconna(
    Alconna("mai {version:str}歌曲表"), use_cmd_start=True, rule=is_allowed
)

level_and_plate_completion_cmd = on_alconna(
    Alconna("mai {keyword:str}完成表"), use_cmd_start=True, rule=is_allowed
)

level_process_cmd = on_alconna(
    Alconna("mai {keyword:str}进度"), use_cmd_start=True, rule=is_allowed
)
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, result: Arparma):
    ...
    if result.find("定数随歌"):
        ...
        if not difficulty:
            difficulty = ""
        m = await inf_mai_roll_by_decimal(
    ...
    if result.find("牌子条件"):
        ...
        await mai_command.finish(
            MessageSegment.reply(event.message_id) + MessageSegment.image(m.get_image())
        )

    # 命令mai ra计算
    if result.find("ra计算"):
        message_occurred()
        m = await inf_mai_ra_calculating(
    ...
    if result.find("分数线"):
        ...
        if difficulty == "":
            difficulty = "紫"
        m = await inf_mai_score_line_v1(
            difficulty, result.query[int]("分数线.music_id")
        )
        if m.status:
            await mai_command.finish(
                MessageSegment.reply(event.message_id)
    ...
    if result.find("分数列表"):
        ...
        m = await inf_mai_level_score_list_v1(
    ...
    if result.find("score"):
        message_occurred()
        m = await inf_mai_score_v1(
            str(event.user_id), result.query[str]("score.music_id")
        )
        if m.status:
            await mai_command.finish(
                MessageSegment.reply(event.message_id)
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, result: Arparma):
    ...
    if m.status:
        await version_song_list_cmd.finish(
            MessageSegment.reply(event.message_id) + MessageSegment.image(m.get_image())
        )
    else:
        await version_song_list_cmd.finish(
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, result: Arparma):
    ...
    if check_level(keyword):
        await level_and_plate_completion_cmd.finish(
            MessageSegment.reply(event.message_id) + MessageSegment.text("🚧施工中~")
        )
    else:
...
async def _(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent, result: Arparma):
    ...
    keyword = result.header["keyword"]
    if check_level(keyword):
        await level_process_cmd.finish(
            MessageSegment.reply(event.message_id) + MessageSegment.text("🚧施工中~")
        )
