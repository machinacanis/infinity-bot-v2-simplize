...
async def _(event: GroupMessageEvent | PrivateMessageEvent, payload: Match[str]):
    if "还是" in payload.result:
        ...
        if len(choices) == 0:
            message_occurred()
            await choose.finish(
                MessageSegment.reply(event.message_id) + f"建议您爬捏！"
            )
        if len(choices) < 2:
            message_occurred()
            await choose.finish(
                MessageSegment.reply(event.message_id)
                + "请至少提供 2 个选项，并以“还是”隔开"
            )
        # 生成一个随机数
        r = random.random()
        ...
        if r < 0.7:
            ...
            await choose.finish(
                MessageSegment.reply(event.message_id)
                + f"建议您选择{random.choice(choices)}捏！"
            )
        # 30% 概率执行代码块 B
        else:
            ...
            message_occurred()
            await choose.finish(
                MessageSegment.reply(event.message_id) + random.choice(msg)
            )
