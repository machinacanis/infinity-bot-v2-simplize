...
# 素材载入
cover = r"./assets/img/today/"
Font_path = r"./assets/font/"
cover_user = r"./cache/user/"
...
def drawing(uid: str) -> Path:
    ...
    def draw_and_save(title_y, title_j, random_y, random_j):
        ...
        text_width, text_height = get_font_size(F18dsz, Tluck)
        start_x = 270
        ...
        try:
            cover = Image.open(
                rf"./assets/cover/mai/UI_Jacket_{get_cover_len6_id(mai_music.id)}.png"
            ).convert("RGBA")
        except Exception:
            cover = Image.open(r"./assets/cover/mai/UI_Jacket_0000.png")
        cover_resize = cover.resize((180, 180))
        im.paste(cover_resize, (120, 820))
        flag = 0
        title_v = f"{mai_music.title}"
        # 215
        if (get_font_size(F16wmt, title_v))[0] > 970:
            flag = 1
        limit = (  # noqa: E731
            lambda text: text if get_font_size(F16wmt, text)[0] <= 970 else limit(text[:-1])
        )
        ...
        # 215
        if (get_font_size(F12wmy, title_a))[0] > 970:
            flbg = 1
        limit = (  # noqa: E731
            lambda text: text if get_font_size(F12wmy, text)[0] <= 970 else limit(text[:-1])
        )
        ...
        try:
            cover = Image.open(
                rf"./assets/cover/chu/CHU_UI_Jacket_{get_cover_len4_id(chu_music.id)}.png"
            ).convert("RGBA")
        except Exception:
            cover = Image.open(r"./assets/cover/chu/CHU_UI_Jacket_0000.png")
        cover_resize = cover.resize((180, 180))
        ...
        limit = (  # noqa: E731
            lambda text: text
            if get_font_size(F16wmt, text)[0] <= 970
            else limit(text[:-1])
        )
        title_v = limit(title_v)
        if flag == 1:
