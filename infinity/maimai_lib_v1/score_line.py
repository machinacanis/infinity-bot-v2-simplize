...
from .tool import get_cover_len6_id, image_path_to_base64, get_font_size
from nonebot.adapters.onebot.v11 import MessageSegment
#from maimaidx_project import *

def split_text(font: ImageFont.FreeTypeFont, max_len: int, text_: str) -> list[str]:
    split_list = [text_, ]
    while get_font_size(font, split_list[-1])[0] >= max_len:
        point = len(split_list[-1])
        while get_font_size(font, split_list[-1][:point])[0] >= max_len:
            point -= 1
...
async def score_line(chart_id: str, level_index: int):
    ...
    bg_path = os.path.join(
        f'./assets/img/mai/pic/mai-accal-bud-{level_labels3[level_index]}.png')
    bg = Image.open(bg_path).resize((1440, 1870)).convert('RGBA')
    cover = Image.open(
        os.path.join(
            f'./assets/cover/mai/UI_Jacket_{get_cover_len6_id(int(chart_id))}.png')).convert(
        'RGBA').resize((300, 300))
    print(music.type)
    type_path = os.path.join(
        f'./assets/img/mai/pic/{music.type}.png')
    ty = Image.open(type_path).resize((90, 30)).convert('RGBA')

    bg.alpha_composite(cover, (130, 130))  # 封面坐标
    bg.alpha_composite(ty, (133, 400))  # 乐曲种类

    Font_path = rf"./assets/font"
    ...
    shs35_42 = ImageFont.truetype("./assets/font/SourceHanSans_35.otf", 42,
                                  encoding="utf-8")
    shs15_28 = ImageFont.truetype("./assets/font/SourceHanSans_15.otf", 28,
                                  encoding="utf-8")
    text_y_len = get_font_size(shs35_42, music.title)[1]
    ...
    text_y_len = get_font_size(shs15_28, music.artist)[1]
