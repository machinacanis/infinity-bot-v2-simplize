...
def song_data_filter(all_data: dict, target_level: str, target_page: int, music_data: dict,
                     user_level: str) -> dict | None:
    diff_count = 0
    filter_data = []
    ...
    return {"level": target_level, "now_page": target_page, "total_pages": total_pages, "total_records": diff_count,
            "song_static": song_static
...
def draw_mai_lv(draw_data: dict, music_data: dict):
    img = Image.open("./assets/img/mai/lv_score/mai-lvscore-bg.png")
    ...
    #id->right name->left
    HanSans37_48 = ImageFont.truetype("./assets/font/SourceHanSans_37.ttf", 48)
    HanSans35_48 = ImageFont.truetype("./assets/font/SourceHanSans_35.otf", 48)
    HanSans37_32 = ImageFont.truetype("./assets/font/SourceHanSans_37.ttf", 32)
    HanSans15_32 = ImageFont.truetype("./assets/font/SourceHanSans_15.otf", 32)

    img_text.text([250, 42], f'Lv {str(draw_data["level"])}', (0, 0, 0), HanSans37_48)
    img_text.text([720, 56], f'第 {str(draw_data["now_page"])} / {str(draw_data["total_pages"])} 页', (0, 0, 0),
                  HanSans37_32)
    #-13 pixels 难度 页码
    ...
    for single_data in draw_data["data"]:
        ...
        dxstar = return_dx_star(single_data["dxScore"],
                                music_data[str(single_data["song_id"])]["charts"][single_data["level_index"]][
                                    "dxscore"])
        simple_draw(img, f"./assets/img/mai/lv_score/dxstar_{dxstar}.png", [24, 24], [465, 280 + data_index * 50])
        #+11 pixels dx星星
        img_text.text([500, 265 + data_index * 50], str(dxstar), (0, 0, 0), HanSans15_32)
        simple_draw(img, f'./assets/img/mai/lv_score/diff_{single_data["level_index"]}.png', [24, 24],
                    [555, 280 + data_index * 50])
        #+11 pixels 难度
