...
from .tool import get_font_size
from ..userdata_client_v1 import df_client
from .music_data_counter import MusicDataCounter
from pathlib import Path
import os
from .plate_map import MAIPLATE_FILE_ID_MAP, VERSION_MAP, VERSION_DF_MAP, MAI_DELETED_MUSIC_REM,MAI_DELETED_MUSIC_Normal
from . import completion_utils as utils
import asyncio

COMPLETION_BASE_PATH = "./assets/img/mai/plate_completion_base"
PLATE_BASE_PATH = "./assets/img/mai/b50/plate"
FONT_PATH = "./assets/font"
NEW_LEVEL_STATUS_TABLE_PATH = "./assets/img/mai/plate_completion"
...
def generate_base_level_image(version:str,is_mai_version:bool,default_song_list:dict):
    ...
    for level,musics in default_song_list.items():
        if musics:
            ...
            level_image = level_image.resize((100, 45), Image.Resampling.LANCZOS)
            ...
            for index,music_id in enumerate(musics):
                ...
                cover_path = f"./assets/cover/mai/UI_Jacket_{song_id_6}.png"
                cover = Image.open(cover_path).convert('RGBA')
                cover = cover.resize((80, 80), Image.Resampling.LANCZOS)
...
def draw_user_total_counter(base_image:Image.Image,plate_mode,is_mai_version:bool,basic_counter,advanced_counter,expert_counter,master_counter,rem_master_counter):
    ...
    for level_index in range(max_level):
        ...
        fx,fy = get_font_size(tempFont, content)
