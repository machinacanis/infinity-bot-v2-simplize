...
import PIL.Image
import httpx
from nonebot_plugin_alconna.builtins.plugins.help import alias

import infinity
from infinity import get_qq_groups_joined_count, get_memory_usage, get_message_count_last_hour, \
    get_hourly_message_processed, get_running_time, format_timedelta, check_diving_fish, check_fanyu, check_lxns, \
    check_wahlap, CacheManager, connection, MaiDifficulty, check_level, maimai_ra_calculate, split_plate_name, \
    ChuniDifficulty
from infinity.image_v2 import maimai_song_card_img, generate_maimai_filename, get_maimai_shortest_song_id, \
    chunithm_song_card_img, generate_chunithm_filename
...
from infinity.whitelist import add_whitelist_qq_group, remove_whitelist_qq_group, add_banned_qq_user
...
async def inf_ping(bot: OnebotV11Bot, is_admin: bool = False):
    ...
    msg.add_content(f"平均每小时处理消息：{round(get_hourly_message_processed(), 2)} 条")
...
async def inf_mai_help():
    ...
    msg.add_content("https://docs.qq.com/aio/p/scm9oh4ypvgb8cq?p=GTjZy8z9ljU8DTngs7AY3wc")
    return msg
...
async def gen_mai_music_text(msg: InfinityUniMessage, music_id: int):
    # 生成文字信息
    # 获取封面
    cover = PIL.Image.open(f"./assets/cover/mai/{generate_maimai_filename(get_maimai_shortest_song_id(music_id))}")
    msg.add_image(cover)
    # 根据信息生成文本
    music_detail = await connection.query_maimai_song_by_id(get_maimai_shortest_song_id(music_id) if music_id <= 100000 else music_id)
    if music_detail:
        is_dx = music_id > 10000
        if is_dx:
            levels_str = ', '.join([chart.level for chart in music_detail.dx_charts])
            decimal_str = ', '.join([str(chart.level_value) for chart in music_detail.dx_charts])
        else:
            levels_str = ', '.join([chart.level for chart in music_detail.sd_charts])
            decimal_str = ', '.join([str(chart.level_value) for chart in music_detail.sd_charts])
        msg.success()
...
async def gen_mai_chart_text(msg: InfinityUniMessage, difficulty: MaiDifficulty, music_id: int):
    """
    生成舞萌谱面信息
    """
    # 生成文字信息
    ...
    if music_detail:
        is_dx = music_id > 10000
        if is_dx and len(music_detail.dx_charts) >= difficulty.int() + 1 and difficulty.int() != -1:
            # 难度存在，获取其信息
            chart = music_detail.dx_charts[difficulty.int()]
        elif not is_dx and len(music_detail.sd_charts) >= difficulty.int() + 1 and difficulty.int() != -1:
            # 难度存在，获取其信息
        ...
        msg.add_content("谱面类型：DX" if is_dx else "谱面类型：标准")
        msg.add_content("BPM：" + str(music_detail.bpm))
        msg.add_content(f"难度：{difficulty.str()} {chart.level}（{chart.level_value}）")
...
async def inf_mai_decimal(difficulty: str, decimal: float):
    ...
    music_list = await connection.query_maimai_song_by_decimal_and_difficulty(decimal, difficulty)
    if len(music_list) == 0:
        msg.add_content("没有搜索到任何结果。")
    elif len(music_list) == 1:
        msg = await gen_mai_music_card(msg, music_list[0].id)
        ...
        for music in music_list:
            ...
            for chart in music.dx_charts if music.id > 10000 else music.sd_charts:
                if chart.level_value == decimal and (difficulty.int() == -1 or chart.difficulty == difficulty.int()):
                    msg.add_content(f"{music.id} {MaiDifficulty(chart.difficulty).str()} {music.title}")
    return msg
...
async def inf_mai_level(difficulty: str, level: str):
    ...
    if is_difficulty:
        ...
        difficulty = MaiDifficulty(-1)
    music_list = await connection.query_maimai_song_by_level_and_difficulty(level, difficulty)
    if len(music_list) == 0:
        msg.add_content("没有搜索到任何结果。")
    elif len(music_list) == 1:
        msg = await gen_mai_music_card(msg, music_list[0].id)
        ...
        for music in music_list:
            # 检查其具体谱面信息
            for chart in music.dx_charts if music.id > 10000 else music.sd_charts:
                if chart.level == level and (difficulty.int() == -1 or chart.difficulty == difficulty.int()):
                    msg.add_content(f"{music.id} {MaiDifficulty(chart.difficulty).str()} {music.title}")
    return msg
...
async def inf_mai_roll_by_decimal(difficulty: str, decimal: float):
    ...
    if is_difficulty:
        ...
        difficulty = MaiDifficulty(-1)
    music = await connection.roll_maimai_song_by_decimal_and_difficulty(decimal, difficulty)
    if music:
        music_id = music.id
        msg = await gen_mai_music_card(msg, music_id)
    else:
...
async def inf_mai_b50_v1(username: str = "", qq: str = "", is_lxns: bool = False, friend_code: str = ""):
    """
    生成Best 50图片
    """
    msg = create_infinity_message()
    if is_lxns:
        ...
        other_data = [player_data["data"]["name"], {'name': 'maimai DX Rating Information', 'color': 'Normal'}, player_data["data"]["course_rank"],
                      0]
    else:
        if username == "" and qq == "":
        ...
        if username:
            ...
            payload["qq"] = qq
        req = httpx.post("https://www.diving-fish.com/api/maimaidxprober/query/player", json=payload)
        ...
        other_data = [player_data["nickname"], {'name': 'maimai DX Rating Information', 'color': 'Normal'}, course_rank, 0,
                      {'name': player_data["plate"]}, None, None]
...
async def inf_mai_ap50_v1(username: str = "", qq: str = "", is_lxns: bool = False, friend_code: str = ""):
    """
    生成AP Best 50图片
    """
    msg = create_infinity_message()
    if is_lxns:
        ...
        if len(ap_b35_data) == 0 and len(ap_b15_data) == 0:
            ...
            return msg
        player_data = {"additional_rating": player_b50_data["additional_rating"],
                       "charts": {"dx": ap_b15_data, "sd": ap_b35_data},
                       "nickname": player_b50_data["nickname"], "plate": player_b50_data["plate"],
        ...
        if course_rank >= 11:
            course_rank = player_data["additional_rating"] + 1
        other_data = [player_data["nickname"], {'name': 'maimai DX Rating Information', 'color': 'Normal'}, course_rank,
                      0,
                      {'name': player_data["plate"]}, None, None]
    # noinspection PyTypeChecker
    # 蘑菇头的代码我真的看不懂，我也不知道为什么这为什么能跑起来，总之看见标黄难受
...
async def gen_chu_music_text(msg: InfinityUniMessage, music_id: int):
    ...
    if music_detail:
        ...
        decimal_str = ', '.join([str(chart.level_value) for chart in music_detail.charts])
        msg.success()
        msg.add_content(str(music_id) + ". " + music_detail.title)
        msg.add_content("艺术家：" + music_detail.artist)
...
async def gen_chu_chart_text(msg: InfinityUniMessage, difficulty: ChuniDifficulty, music_id: int):
    """
    生成舞萌谱面信息
    """
    # 生成文字信息
...
async def inf_chu_decimal(difficulty: str, decimal: float):
    ...
    music_list = await connection.query_chunithm_song_by_decimal_and_difficulty(decimal, difficulty)
    if len(music_list) == 0:
        msg.add_content("没有搜索到任何结果。")
    elif len(music_list) == 1:
        msg = await gen_chu_music_card(msg, music_list[0].id)
        ...
        for music in music_list:
            ...
            for chart in music.charts :
                if chart.level_value == decimal and (difficulty.int() == -1 or chart.difficulty == difficulty.int()):
                    msg.add_content(f"{music.id} {ChuniDifficulty(chart.difficulty).str()} {music.title}")
    return msg
...
async def inf_chu_level(difficulty: str, level: str):
    ...
    if is_difficulty:
        ...
        difficulty = ChuniDifficulty(-1)
    music_list = await connection.query_chunithm_song_by_level_and_difficulty(level, difficulty)
    if len(music_list) == 0:
        msg.add_content("没有搜索到任何结果。")
    elif len(music_list) == 1:
        msg = await gen_chu_music_card(msg, music_list[0].id)
...
async def inf_chu_roll_by_decimal(difficulty: str, decimal: float):
    ...
    if is_difficulty:
        ...
        difficulty = ChuniDifficulty(-1)
    music = await connection.roll_chunithm_song_by_decimal_and_difficulty(decimal, difficulty)
    if music:
        music_id = music.id
        msg = await gen_chu_music_card(msg, music_id)
    else:
...
async def inf_chu_roll_by_level(difficulty: str, level: str):
    ...
    if is_difficulty:
        ...
        difficulty = ChuniDifficulty(-1)
    music = await connection.roll_chunithm_song_by_level_and_difficulty(level, difficulty)
    if music:
        music_id = music.id
        msg = await gen_chu_music_card(msg, music_id)
    return msg
