...
class UtageChartData:
    ...
    def to_dict(self):
        ...
        if self.is_buddy:
            ...
            data["right_notes"] = (
                self.right_notes.to_dict() if self.right_notes else None
            )
    ...
    def from_dict(data: dict) -> "UtageChartData":
        ...
        left_notes = (
            Notes.from_dict(data.get("left_notes", {}))
            if "left_notes" in data
            else None
        )
        right_notes = (
            Notes.from_dict(data.get("right_notes", {}))
            if "right_notes" in data
            else None
        )
...
class MaimaiSongData:
    ...
    def from_dict(data: dict) -> "MaimaiSongData":
        ...
        sd_charts = [
            MaimaiChartData.from_dict(chart) for chart in data.get("sd_charts", [])
        ]
        dx_charts = [
            MaimaiChartData.from_dict(chart) for chart in data.get("dx_chart", [])
        ]
        utage_charts = [
            UtageChartData.from_dict(chart) for chart in data.get("utage_chart", [])
        ]
...
def create_utage_chart_data(data, type_str):
    ...
    left_notes = (
        create_notes(data.get("notes", {}).get("left", {}))
        if "left" in data.get("notes", {})
        else None
    )
    right_notes = (
        create_notes(data.get("notes", {}).get("right", {}))
        if "right" in data.get("notes", {})
        else None
    )
