...
async def maimai_song_card_img(song_id: int) -> BytesIO | None:
    ...
    # 没有缓存则开始生成图片
    # 首先通过数据库获取这首曲目的完整信息
    song = await connection.query_maimai_song_by_id(
        get_maimai_shortest_song_id(song_id) if song_id <= 100000 else song_id
    )
    print(song)
    # 判断是否获取到了歌曲信息
    if song is None:
    ...
    if not is_utage:
        # 根据是否包含白谱来选择不同的图片模板
        cover = Image.open(
            f"./assets/cover/mai/{generate_maimai_filename(get_maimai_shortest_song_id(song_id))}"
        )
        cover = cover.resize((300, 300))
        badge = Image.open(
            f"./assets/img/{'id-deluxe' if is_dx else 'id-standard'}.png"
        ).convert("RGBA")
        ...
        draw.text(
            (460, 395),
            f"ID {song_id}          {song.genre_cn}          BPM: {song.bpm}",
            font=sans_17_font,
            fill="#1E3663",
        )
        # 开始绘制谱面数据信息
        ...
        draw_centered_text(
            draw,
            str("-" if not is_dx else str(charts[0].notes.touchs)),
            (1090, 655),
            ntlgpro_B,
            "#1E3663",
        )
        draw_centered_text(
            draw,
            str("-" if not is_dx else str(charts[1].notes.touchs)),
            (1090, 735),
            ntlgpro_B,
            "#1E3663",
        )
        draw_centered_text(
            draw,
            str("-" if not is_dx else str(charts[2].notes.touchs)),
            (1090, 815),
            ntlgpro_B,
            "#1E3663",
        )
        draw_centered_text(
            draw,
            str("-" if not is_dx else str(charts[3].notes.touchs)),
            (1090, 895),
            ntlgpro_B,
            "#1E3663",
        )
        if is_remaster:
            draw_centered_text(
                draw,
                str("-" if not is_dx else str(charts[4].notes.touchs)),
                (1090, 995),
                ntlgpro_B,
                "#1E3663",
            )
        ...
        # 绘制BREAK列
        draw_centered_text(
            draw, str(charts[0].notes.breaks), (1270, 655), ntlgpro_B, "#1E3663"
        )
        draw_centered_text(
            draw, str(charts[1].notes.breaks), (1270, 735), ntlgpro_B, "#1E3663"
        )
        draw_centered_text(
            draw, str(charts[2].notes.breaks), (1270, 815), ntlgpro_B, "#1E3663"
        )
        draw_centered_text(
            draw, str(charts[3].notes.breaks), (1270, 895), ntlgpro_B, "#1E3663"
        )
        if is_remaster:
            draw_centered_text(
                draw, str(charts[4].notes.breaks), (1270, 995), ntlgpro_B, "#1E3663"
            )
        ...
        draw.text(
            (1148, 116), song.utage_charts[0].kanji, font=ntlgpro_B_18, fill="#FFFFFF"
        )
        ...
        draw_centered_text(
            draw,
            song.utage_charts[0].description,
            (720, 625),
            sans_35_font_28,
            "#FFFFFF",
        )
        ...
        draw_centered_text(
            draw,
            str(
                song.utage_charts[0].normal_notes.total
                if not is_buddy
                else song.utage_charts[0].left_notes.total
            ),
            (370, 865),
            ntlgpro_EB,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.total),
                (370, 945),
                ntlgpro_EB,
                "#FFFFFF",
            )
        ...
        draw_centered_text(
            draw,
            str(
                song.utage_charts[0].normal_notes.taps
                if not is_buddy
                else song.utage_charts[0].left_notes.taps
            ),
            (550, 865),
            ntlgpro_B,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.taps),
                (550, 945),
                ntlgpro_B,
                "#FFFFFF",
            )
        ...
        draw_centered_text(
            draw,
            str(
                song.utage_charts[0].normal_notes.holds
                if not is_buddy
                else song.utage_charts[0].left_notes.holds
            ),
            (730, 865),
            ntlgpro_B,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.holds),
                (730, 945),
                ntlgpro_B,
                "#FFFFFF",
            )
        ...
        draw_centered_text(
            draw,
            str(
                song.utage_charts[0].normal_notes.slides
                if not is_buddy
                else song.utage_charts[0].left_notes.slides
            ),
            (910, 865),
            ntlgpro_B,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.slides),
                (910, 945),
                ntlgpro_B,
                "#FFFFFF",
            )
        ...
        draw_centered_text(
            ...
            str(
                song.utage_charts[0].normal_notes.slides
                if not is_buddy
                else song.utage_charts[0].left_notes.touchs
            ),
            (1090, 865),
            ntlgpro_B,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.touchs),
                (1090, 945),
                ntlgpro_B,
                "#FFFFFF",
            )
        ...
        draw_centered_text(
            ...
            str(
                song.utage_charts[0].normal_notes.slides
                if not is_buddy
                else song.utage_charts[0].left_notes.breaks
            ),
            (1270, 865),
            ntlgpro_B,
            "#FFFFFF",
        ...
        if is_buddy:
            draw_centered_text(
                draw,
                str(song.utage_charts[0].right_notes.breaks),
                (1270, 945),
                ntlgpro_B,
                "#FFFFFF",
            )
        ...
        if is_buddy:
            draw_centered_text(
                draw, song.utage_charts[0].level, (540, 1110), ntlgpro_EB_48, "#FFFFFF"
            )
            draw_centered_text(
                draw, song.version_name, (820, 1140), sans_37_font, "#FFFFFF"
            )
        else:
            draw_centered_text(
                draw, song.utage_charts[0].level, (540, 1040), ntlgpro_EB_48, "#FFFFFF"
            )
            draw_centered_text(
                draw, song.version_name, (820, 1070), sans_37_font, "#FFFFFF"
            )
