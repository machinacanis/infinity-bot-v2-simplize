...
from .utils import get_maimai_shortest_song_id, generate_maimai_filename, sans_35_font, sans_15_font, ntlgpro_B, \
    ...
    sans_17_font, ntlgpro_EB_48, generate_chunithm_filename
...
async def chunithm_song_card_img(song_id: int):
    ...
    if not is_worldend:
        ...
        draw.text((460, 395), f"ID {song_id}          {song.genre}          BPM: {song.bpm}", font=sans_17_font,
                  fill="#1E3663")
        # 开始绘制谱面数据信息
        charts = song.charts
        ...
        # 绘制Air列
        draw_centered_text(draw, str(charts[0].notes.airs), (1090, 655), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[1].notes.airs), (1090, 735), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[2].notes.airs), (1090, 815), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[3].notes.airs), (1090, 895), ntlgpro_B, "#1E3663")
        if is_ultima:
            draw_centered_text(draw, str(charts[4].notes.airs), (1090, 995), ntlgpro_B, "#1E3663")

        # 绘制Flick列
        draw_centered_text(draw, str(charts[0].notes.flicks), (1270, 655), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[1].notes.flicks), (1270, 735), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[2].notes.flicks), (1270, 815), ntlgpro_B, "#1E3663")
        draw_centered_text(draw, str(charts[3].notes.flicks), (1270, 895), ntlgpro_B, "#1E3663")
        if is_ultima:
            draw_centered_text(draw, str(charts[4].notes.flicks), (1270, 995), ntlgpro_B, "#1E3663")
        ...
        # worldend谱面
        cover = Image.open(f"./assets/cover/chuwe/{generate_chunithm_filename(song_id)}")
        cover = cover.resize((300, 300))
        ...
        # 歌曲信息
        draw_centered_text(draw, f"{song.charts[0].kanji} ☆{song.charts[0].star}", (181, 668), ntlgpro_B, "#FFFFFF")

        # 绘制谱面数据
        # TOTAL列
        draw_centered_text(draw, str(
            song.charts[0].notes.total),
                           (370, 655), ntlgpro_EB, "#1E3663")
        # TAP列
        draw_centered_text(draw, str(
            song.charts[0].notes.taps),
                           (550, 655), ntlgpro_B, "#1E3663")

        # HOLD列
        draw_centered_text(draw, str(
            song.charts[0].notes.holds),
                           (730, 655), ntlgpro_B, "#1E3663")

        # SLIDE列
        draw_centered_text(draw, str(
            song.charts[0].notes.slides),
                           (910, 655), ntlgpro_B, "#1E3663")

        # Air列
        draw_centered_text(draw, str(
            song.charts[0].notes.airs),
                           (1090, 655), ntlgpro_B, "#1E3663")

        # Flick列
        draw_centered_text(draw, str(
            song.charts[0].notes.flicks),
                           (1270, 655), ntlgpro_B, "#1E3663")

        # 谱面作者
        draw_wrapped_text(draw, song.charts[0].note_designer, (300, 780), sans_15_font_28, "#1E3663", 1035)
